...
export async function stackRemove(name, opts) {
    ...
    if (activeProfiles.length > 0) {
        console.log(kleur.yellow(`Warning: the following profile(s) reference stack "${name}": ${activeProfiles.join(', ')}.`));
    ...
    if (activeProfiles.length > 0) {
        console.log(kleur.gray(`→ Update the stacks list in affected profile(s) to avoid install errors.`));
    }
}
function profilesReferencingStack(stackName) {
