 ...
 */
export async function off(profileName: string, opts: ToggleOptions): Promise<void> {
  ...
  p.log.success(
    `Paused. Resume with ${kleur.cyan(`atb on ${profileName}`)} — the same surfaces will be re-enabled.`
  );
}
 ...
 */
export async function on(profileName: string, opts: ToggleOptions): Promise<void> {
  const state = readState();
  const profile = state.profiles[profileName];
  ...
  if (profile && Object.keys(profile.surfaces).length > 0) {
    p.log.warn(
      `Profile "${profileName}" is already active on: ${Object.keys(profile.surfaces).join(', ')}.`
    );
    return;
  }
  ...
  if (!paused || Object.keys(paused).length === 0) {
    throw new Error(
      `Profile "${profileName}" has no paused state. Use \`atb install ${profileName}\` for the first-time setup.`
    );
  }
