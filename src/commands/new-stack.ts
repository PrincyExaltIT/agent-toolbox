 ...
 */
export async function newStack(name: string, opts: NewStackOptions): Promise<void> {
  ...
  if (interactive) {
    ...
    throw new Error(
      'Non-TTY without --yes and missing flags. Pass --description and --files, or use --yes.'
    );
  }

  if (!description) description = `${name} coding conventions`;
  if (files.length === 0) files = [`${name}-coding-guidelines.md`];
