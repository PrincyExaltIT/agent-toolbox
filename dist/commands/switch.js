 ...
 */
export async function switchProfile(profileName, opts) {
    ...
    for (const [, st] of others) {
        for (const surface of Object.keys(st.surfaces)) {
            surfaceSet.add(surface);
        }
    }
    p.log.info(`Switching to ${kleur.bold(profileName)} on surfaces: ${[...surfaceSet].join(', ')}`);
    // 1. Uninstall each other profile from the affected surfaces.
    for (const [otherName, st] of others) {
        const surfaces = Object.keys(st.surfaces);
