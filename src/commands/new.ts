...
export async function newProfile(name: string, opts: NewOptions): Promise<void> {
  ...
  if (shared.length === 0 && stacks.length === 0) {
    throw new Error(
      'No shared guidelines or stacks in your content root yet. Run `atb new shared <name>` or `atb new stack <name>` first, or import an existing tree via `atb config init --from-path <dir>`.'
    );
  }
  ...
  if (interactive) {
    ...
    if (!copilotDescription) {
      ...
      if (p.isCancel(ans)) return cancel();
      copilotDescription = (ans as string) || description || `${name} agent`;
    }
  } else if (!opts.yes) {
  ...
  if (!description) description = `${name} profile`;
  if (pickedShared.length === 0) pickedShared = shared.slice(0, 2).map((s) => s.file);
  // stacks default empty — user probably wants to pick explicitly
  if (!copilotDescription) copilotDescription = description;

  // Validate picks.
  const sharedSet = new Set(shared.map((s) => s.file));
  for (const f of pickedShared) {
    if (!sharedSet.has(f)) throw new Error(`Unknown shared guideline "${f}"`);
  }
  const stackSet = new Set(stacks.map((s) => s.name));
  for (const s of pickedStacks) {
    if (!stackSet.has(s)) throw new Error(`Unknown stack "${s}" — create it under ~/.agent-toolbox/stacks/${s}/ first.`);
  }
  ...
  fs.writeFileSync(
    ...
    renderProjectContext(name, description)
  );
  fs.writeFileSync(
    path.join(dir, 'CLAUDE.md'),
    renderProfileClaudeMd(name, pickedShared, [stackImports].filter(Boolean))
  );
