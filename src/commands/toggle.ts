...
export type ToggleOptions = Omit<
  InstallOptions,
  'uninstall' | 'all' | 'claude' | 'copilotVs' | 'copilotVscode' | 'copilotCli' | 'codex' | 'surfaces'
>;
 ...
 */
export async function off(profileName: string | undefined, opts: ToggleOptions): Promise<void> {
  ...
  const active = Object.keys(profile.surfaces) as SurfaceName[];
  ...
  p.log.success(
    `Paused. Resume with ${kleur.cyan('atb on')} — the same surfaces will be re-enabled.`
  );
}
 ...
 */
export async function on(profileName: string | undefined, opts: ToggleOptions): Promise<void> {
  const state = readState();
  ...
  if (profile && Object.keys(profile.surfaces).length > 0) {
    p.log.warn(
      `Profile "${resolved}" is already active on: ${Object.keys(profile.surfaces).join(', ')}.`
    );
    return;
  }
  ...
  if (!paused || Object.keys(paused).length === 0) {
    throw new Error(
      `Profile "${resolved}" has no paused state. Use \`atb install ${resolved}\` for the first-time setup.`
    );
  }

  const toEnable = Object.keys(paused) as SurfaceName[];
...
function resolveActive(state: ToolboxState): string {
  const active = Object.entries(state.profiles).filter(
    ([, prof]) => Object.keys(prof.surfaces).length > 0
  );
  if (active.length === 0) {
    throw new Error(
      'No profile is currently active. Use `atb install <profile>` first, or `atb on <profile>` to resume a paused one.'
    );
  }
  if (active.length > 1) {
...
function resolvePaused(state: ToolboxState): string {
  const paused = Object.entries(state.profiles).filter(
    ([, prof]) =>
      Object.keys(prof.surfaces).length === 0 &&
      prof.pausedSurfaces &&
      Object.keys(prof.pausedSurfaces).length > 0
  );
  if (paused.length === 0) {
    throw new Error(
      'No paused profile to resume. Use `atb install <profile>` for a first-time setup, or `atb list` to see what is available.'
    );
  }
  if (paused.length > 1) {
