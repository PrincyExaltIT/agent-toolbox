...
export function status(opts: StatusOptions = {}): void {
  ...
  for (const name of profileNames) {
    ...
    const activeSurfaces = Object.keys(prof.surfaces) as SurfaceName[];
    const pausedSurfaces = Object.keys(prof.pausedSurfaces ?? {}) as SurfaceName[];
  ...
  if (driftCount > 0) {
    ...
    const paused = profileNames.filter(
      (n) =>
        Object.keys(state.profiles[n].surfaces).length === 0 &&
    ...
    if (parts.length > 0) console.log(kleur.gray(parts.join(', ') + '.'));
  }
}
